...
- **통화기록**: 시작 화면, 최신순 목록, 전체·부재중·태그 있음 필터, 검색, 100건 단위 추가 조회. 검색·필터 결과에서 같은 번호가 연속되면 하나로 묶고 2건 이상은 이름 옆에 횟수를 표시합니다. 통화 유형과 시간은 묶음의 최신 기록을 사용합니다.
...
- **기록 팝업**: 선택한 번호의 통화기록을 처음 5건 표시하고, 더보기마다 10건씩 추가합니다. 기기 조회는 기존 100건 단위를 유지하며 불러온 기록을 재사용합니다. 최근 SMS 50건의 미리보기는 누르면 기본 메시지 앱에서 원문 열기를 요청합니다. 메시지·대화 URI를 지원하지 않으면 해당 번호의 대화창을 열며, 특정 문자 위치로 이동하는지는 기본 메시지 앱의 지원에 따라 달라집니다. 사용자가 연결한 통화 녹음 파일은 기본 재생 앱에서 엽니다.
- **환경설정**: 통화기록 상단 문구, 화면별 사진 배경, 글자 크기 85~140%, 글자·배경 색상, 기존 태그 자동 추천 설정. 상단 문구는 줄바꿈을 포함해 저장할 수 있으며 비워서 저장하면 기본 문구로 돌아갑니다. 기기의 접근성 글자 배율도 함께 적용합니다.
